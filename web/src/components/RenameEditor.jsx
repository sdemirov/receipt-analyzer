...
export default function RenameEditor() {
  ...
  const [edits, setEdits] = useState({}); // product_id -> new value
    ...
    <div className="panel rename">
      ...
      <table className="rename-table">
        ...
        <tbody>
          {shown.map((r) => {
            ...
            const toSave = val.trim() === r.auto_name ? "" : val; // auto name => no override
            return (
          ...
          {rows.length === 0 && (
            <tr><td colSpan={4} className="muted">Няма резултати</td></tr>
          )}
        </tbody>
      </table>
      ...
      {rows.length > 0 && (
        <div className="pager">
          ...
          <span>Стр. {cur + 1} / {pageCount} · {rows.length} продукта</span>
